...
CRCs are skipped, and parameters for [`read()`](../shared#parsed) are left at defaults (frame 0, RGBA8 output)
...
- **16-bit depth:** Supported. Use a 16-bit color target in `read()` to preserve full color depth.
